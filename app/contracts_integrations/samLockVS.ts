...
import { ERC20_ABI, SAM_LOCK_V3_ABI, SAM_LOCK_VS_ABI } from "./abis";
...
export async function userInfo(account: string) {
  try {
    ...
    let balance = Number(
      ethers.formatEther(
        await balanceOf(ERC20_ABI, SAM_SONIC_ADDRESS, address, provider)
      )
    );
...
export async function requestPoints(lockIndex: number, signer: ethers.Signer) {
  try {
    const contract = await getContract(signer);
    const network = await signer.provider?.getNetwork();

    const tx = await contract?.requestPointsFor(lockIndex);
...
export async function fulfillRequests(lockIndex: number, signer: ethers.Signer) {
  try {
    const contract = await getContract(signer);
    const network = await signer.provider?.getNetwork();
